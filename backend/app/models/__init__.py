...
class Listing(Base):
    ...
    stock_status = Column(SAEnum(StockStatus), default=StockStatus.unknown)
...
class ScrapeJob(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    store_id = Column(Integer, ForeignKey("stores.id"))
    status = Column(SAEnum(ScrapeJobStatus), default=ScrapeJobStatus.pending)
...
class MatchReviewQueue(Base):
    ...
    status = Column(SAEnum(MatchReviewStatus), default=MatchReviewStatus.pending)
    created_at = Column(DateTime(timezone=True), server_default=func.now())
