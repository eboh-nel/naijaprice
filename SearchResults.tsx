...
def _build_search_filter(query: str):
    """
    Build a text search filter that works on both PostgreSQL and SQLite.
    Uses PostgreSQL full-text search when available, falls back to ILIKE.
    """
    terms = re.sub(r"[^\w\s]", " ", query.lower()).split()
    conditions = []
    for term in terms:
        conditions.append(Product.normalized_name.ilike(f"%{term}%"))
        conditions.append(Product.brand.ilike(f"%{term}%"))
        conditions.append(Product.model.ilike(f"%{term}%"))
