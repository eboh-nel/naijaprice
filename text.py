...
@router.get("/{slug}/variants", response_model=list[VariantOut])
def get_variants(slug: str, db: Session = Depends(get_db)):
    """Return all variants for a product slug — used by VariantSelector on the frontend."""
    product = db.query(Product).filter(Product.slug == slug).first()
    if not product:
        raise HTTPException(status_code=404, detail="Product not found")
    return db.query(ProductVariant).filter(ProductVariant.product_id == product.id).all()
