...
from app.models import MatchReviewQueue, MatchReviewStatus, ScrapeJob, ScrapeJobStatus, Listing, ProductVariant
from app.tasks.scrape_tasks import run_scraper
...
def get_unmatched(limit: int = 50, db: Session = Depends(get_db)):
    ...
    result = []
    for item in items:
        listing = db.query(Listing).get(item.listing_id)
        result.append({
            "review_id": item.id,
...
def approve_match(review_id: int, variant_id: int, db: Session = Depends(get_db)):
    item = db.query(MatchReviewQueue).get(review_id)
    if not item:
...
def trigger_scraper(store_slug: str):
    # Fire-and-forget via Celery task
